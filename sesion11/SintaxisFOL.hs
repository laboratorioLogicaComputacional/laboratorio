...
varLibresDe :: FOL -> [Id]
varLibresDe phi = case phi of
  Pred _ lt -> concat $ map varT lt
  Equ (p,n) lt -> concat $ map varT lt
  Oimp a b -> varLibresDe a ++ varLibresDe b
  Oand a b -> varLibresDe a ++ varLibresDe b
  Oor a b -> varLibresDe a ++ varLibresDe b
  Oneg a -> varLibresDe a
  All x a -> elimn x (varLibresDe a)
  Exi x a -> elimn x (varLibresDe a)
  Top -> []
  Bot -> []
...
-- Variables ligadas de una formula
varLigadasDe :: FOL -> [Id]
varLigadasDe phi = case phi of
  Pred _ lt -> []
  Equ (p,n) lt -> []
  Oimp a b -> varLigadasDe a ++ varLigadasDe b
  Oand a b -> varLigadasDe a ++ varLigadasDe b
  Oor a b -> varLigadasDe a ++ varLigadasDe b
  Oneg a -> varLigadasDe a
  All x a -> [x] ++ varLigadasDe a
  Exi x a -> [x] ++ varLigadasDe a
  Top -> []
  Bot -> []

-- Función que nos indica si una varibale esta libre en una fórmula
ocurreLibreEn :: Id -> FOL -> Bool
ocurreLibreEn x phi = elem x (varLibresDe phi)

-- Función que nos indica si una variable esta ligada en una fórmula
ocurreLigadaEn :: Id -> FOL -> Bool
ocurreLigadaEn x phi = elem x (varLigadasDe phi)


{-
b3) Construir phi y x tales que: ocurreLibreEn(x,phi)=True y ocurreLigadaEn(x,phi)=False
b4) Construir phi y x tales que: ocurreLibreEn(x,phi)=False y ocurreLigadaEn(x,phi)=True
b5) Construir phi y x tales que: ocurreLibreEn(x,phi)=True y ocurreLigadaEn(x,phi)=True
b6) Construir phi y x tales que: ocurreLibreEn(x,phi)=False y ocurreLigadaEn(x,phi)=False
